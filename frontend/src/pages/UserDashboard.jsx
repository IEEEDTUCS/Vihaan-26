...
function CheckpointRow({ cp, token, onAlert, isLeader }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!/^https:\/\/github\.com\/[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+\/commit\/[a-f0-9]{7,40}$/.test(commitLink.trim())) {
      onAlert({ flags:["Please provide a valid GitHub commit URL (https://github.com/owner/repo/commit/sha)"], severity:3 });
      return;
    }
    setSubmitting(true);
    try {
...
function InitialSubmissionForm({ team, token, onAlert, windowOpen }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (repoLink && !/^https:\/\/github\.com\/[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+(\.git)?\/?$/.test(repoLink)) {
      onAlert({ flags:["Please provide a valid GitHub repo URL."], severity:3 });
      setSaving(false); return;
    }
      ...
      {team && (
        <div style={{ background:"rgba(255,255,255,0.03)", border:"1px solid #333", borderRadius:"0.5rem", padding:"14px 16px" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(140px, 1fr))", gap:"8px 16px" }}>
            {[["Team ID",team.team_id],["Type",team.type||"—"],["Room No.",team.room_number||"—"],
              ["Panel",team.panel_number||"—"],["Avg Points",team.avg_points??"—"],["Stars",team.stars??"—"]
            ].map(([k,v]) => (
              <div key={k}>
