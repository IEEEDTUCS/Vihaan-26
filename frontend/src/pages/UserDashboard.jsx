...
// ── Time windows (must match backend .env) ────────────────────────────────────
const WINDOWS = {
  initial: { open: new Date("2026-04-11T14:00:00+05:30"), close: new Date("2026-04-11T16:00:00+05:30") },
  1: { open: new Date("2026-04-11T18:00:00+05:30"), close: new Date("2026-04-11T19:00:00+05:30") },
  2: { open: new Date("2026-04-11T21:00:00+05:30"), close: new Date("2026-04-11T22:00:00+05:30") },
  3: { open: new Date("2026-04-12T00:00:00+05:30"), close: new Date("2026-04-12T01:00:00+05:30") },
  4: { open: new Date("2026-04-12T03:00:00+05:30"), close: new Date("2026-04-12T04:00:00+05:30") },
};
  ...
  return (
    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.35 }}
      ...
      <div onClick={() => setOpen(o => !o)}
        style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 18px", backgroundColor: bgColor, border: open ? `0.3em solid ${borderColor}` : "none", borderRadius: open ? "0.5rem 0.5rem 0 0" : "0", cursor: "pointer", userSelect: "none", position: "relative", overflow: "hidden" }}>
        ...
        <span className="heading" style={{ fontSize: "clamp(1rem, 3vw, 1.7rem)", letterSpacing: "0.2rem", WebkitTextStroke: `1px ${borderColor}`, WebkitTextFillColor: textColor, filter: "drop-shadow(3px 3px 1px rgba(0,0,0,1))", backgroundImage: "none", position: "relative", zIndex: 1 }}>{title}</span>
        <div style={{ width: 36, height: 36, background: "black", borderRadius: "7mm", display: "flex", alignItems: "center", justifyContent: "center", color: textColor, fontSize: "1.4rem", fontFamily: "Bangers", flexShrink: 0, transition: "transform 0.2s", transform: open ? "rotate(180deg)" : "none", position: "relative", zIndex: 1 }}>⌄</div>
      </div>
...
function WindowBanner({ window: w, label }) {
  ...
  return (
    <div style={{ fontFamily: "Edu TAS Beginner, sans-serif", fontSize: "0.85rem", color, marginBottom: 14, padding: "6px 10px", border: `1px solid ${color}`, borderRadius: "0.3rem", display: "inline-block" }}>
      {label}: {msg}
    </div>
  );
}
...
// ── Checkpoint row ────────────────────────────────────────────────────────────
function CheckpointRow({ cp, token, onAlert, isLeader }) {
  ...
  return (
    <div style={{ padding: "12px 14px", marginBottom: 10, background: "rgba(255,255,255,0.04)", border: "1px solid #333", borderRadius: "0.4rem" }}>
      <div style={{ display: "flex", alignItems: "center", flexWrap: "wrap", gap: 8, marginBottom: 6 }}>
        ...
        <span style={{ fontFamily: "Bangers", fontSize: "0.95rem", color: statusColor[cp.status] || "#888", border: `1px solid ${statusColor[cp.status] || "#888"}`, borderRadius: "0.3rem", padding: "2px 8px" }}>{cp.status}</span>
        {cp.submit_link && <a href={cp.submit_link} target="_blank" rel="noreferrer" style={{ fontFamily: "Edu TAS Beginner, sans-serif", fontSize: "0.8rem", color: "#9CA802", textDecoration: "underline" }}>View</a>}
      </div>
      {isLeader && !alreadyDone && windowOpen && (
        <form onSubmit={handleSubmit} style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <input type="url" placeholder="https://github.com/owner/repo/commit/sha" value={commitLink} onChange={e => setCommitLink(e.target.value)}
          ...
          <button type="submit" disabled={submitting}
            style={{ fontFamily: "Bangers", fontSize: "1rem", letterSpacing: "0.15em", color: "#000", background: submitting ? "#666" : "#bba75d", border: "none", borderRadius: "0.4rem", padding: "8px 18px", cursor: submitting ? "not-allowed" : "pointer" }}>
      ...
      {isLeader && !alreadyDone && !windowOpen && w && (
        <p style={{ fontFamily: "Edu TAS Beginner, sans-serif", fontSize: "0.8rem", color: "#555", marginTop: 4 }}>
          {Date.now() < w.open.getTime() ? `Submission opens at ${fmtTime(w.open)}` : "Submission window closed"}
        </p>
      )}
    </div>
  );
...
export default function UserDashboard() {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center" style={{ background: "#0d0d0d" }}><span className="heading" style={{ fontSize: "2rem" }}>Loading...</span></div>;
  if (!user && !admin) { navigate("/login"); return null; }

  const activeUser = user;
  const isLeader = activeUser?.role === "LEADER";
  const team = teamData || activeUser?.team || null;
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden font-sans" style={{ background: "#0d0d0d" }}>
      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        {[{pos:"left-6 top-14",size:"2.2rem",delay:0},{pos:"right-10 top-20",size:"3rem",delay:0.4},{pos:"left-[15%] top-[38%]",size:"1.8rem",delay:0.8},{pos:"right-[18%] top-[22%]",size:"2.5rem",delay:1.2},{pos:"left-[8%] top-[60%]",size:"2rem",delay:0.3},{pos:"right-[6%] top-[50%]",size:"1.6rem",delay:1.5}].map((s,i) => (
          <motion.img key={i} src="/star.svg" className={`absolute ${s.pos}`} style={{ width: s.size, opacity: 0.45 }}
            animate={{ opacity:[0.2,0.65,0.2], scale:[1,1.2,1] }} transition={{ duration:2.4, repeat:Infinity, ease:"easeInOut", delay:s.delay }} />
        ))}
      </div>
      ...
      <div className="relative z-10 max-w-3xl mx-auto px-4 pt-14 pb-72">
        <div className="flex items-center justify-between mb-2 flex-wrap gap-3">
          <motion.h1 className="heading" initial={{ opacity:0, y:-20 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.6 }} style={{ fontSize:"clamp(2rem,8vw,5rem)" }}>Dashboard</motion.h1>
          <button onClick={() => { logout(); navigate("/login"); }}
            ...
            onMouseEnter={e => e.target.style.background="rgba(255,68,68,0.1)"} onMouseLeave={e => e.target.style.background="transparent"}>Logout</button>
        </div>
        ...
        <Section title="02 · Checkpoint Submissions" borderColor="#bba75d" bgColor="rgba(187,167,93,0.08)" textColor="#e8c96a">
          {checkpoints.length === 0
            ...
            : checkpoints.map(cp => <CheckpointRow key={cp.round_num} cp={cp} token={token} onAlert={showAlerts} isLeader={isLeader} />)
          }
        </Section>
...
// ── Read-only initial submission (for members) ────────────────────────────────
function ReadOnlyInitial({ team }) {
  ...
  const inputStyle = { fontFamily:"Edu TAS Beginner, sans-serif", fontSize:"0.9rem", color:"#ccc" };
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
      {team.type && <p style={inputStyle}><span style={{ color:"#9CA802", fontFamily:"Bangers", letterSpacing:"0.1em" }}>Type: </span>{team.type}</p>}
      {team.category?.length > 0 && <p style={inputStyle}><span style={{ color:"#9CA802", fontFamily:"Bangers", letterSpacing:"0.1em" }}>Category: </span>{team.category.join(", ")}</p>}
      {team.description && <p style={inputStyle}><span style={{ color:"#9CA802", fontFamily:"Bangers", letterSpacing:"0.1em" }}>Description: </span>{team.description}</p>}
      {team.repo_or_image_link && (
        <p style={inputStyle}><span style={{ color:"#9CA802", fontFamily:"Bangers", letterSpacing:"0.1em" }}>Repo/Image: </span>
...
function InitialSubmissionForm({ team, token, onAlert, windowOpen }) {
  ...
  if (alreadySubmitted) {
    return <ReadOnlyInitial team={team} />;
  }

  if (!windowOpen) {
    return <p style={{ fontFamily:"Edu TAS Beginner, sans-serif", color:"#888" }}>Submission window is not open yet.</p>;
  }
    ...
    <div>
      ...
      {team && (
        <div style={{ background:"rgba(255,255,255,0.03)", border:"1px solid #333", borderRadius:"0.5rem", padding:"14px 16px" }}>
          <p style={{ fontFamily:"Bangers", fontSize:"1.2rem", color:"#bba75d", letterSpacing:"0.15em", marginBottom:10 }}>Team: {team.team_name}</p>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(140px, 1fr))", gap:"8px 16px" }}>
            {[["Team ID",team.team_id],["Type",team.type||"—"],["Room No.",team.room_number||"—"],["Panel",team.panel_number||"—"],["Avg Points",team.avg_points??"—"],["Stars",team.stars??"—"]].map(([k,v]) => (
              <div key={k}>
          ...
          {team.category?.length > 0 && <p style={{ fontFamily:"Edu TAS Beginner, sans-serif", fontSize:"0.85rem", color:"#888", marginTop:8 }}>Categories: {team.category.join(", ")}</p>}
        </div>
      )}
    </div>
  );
